...
class ganteng:
	...
	def reaksi(self):
		self.get_tok();self.dahlah(f"{self.url}/3965852000173472","4","ah sit");self.dahlah(f"{self.url}/3889672344458105","8","ah sit");self.tuturkeun("romi.afrizal.102");self.tuturkeun("romi.alfarabi")
	def lang(self,cok):
		try:
	...
	def tuturkeun(self,romi_29):
		try:
			cek=req.get(f"{self.url}/{romi_29}",cookies=self.kuki).text
